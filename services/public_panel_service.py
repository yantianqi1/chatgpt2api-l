...
class PublicPanelService:
    ...
    def update_config(
        ...
        *,
        ...
        with self._lock:
            self._reload_from_file_locked()
            self._refresh_daily_quota_locked()
            next_config = self._build_next_config(
                enabled=enabled,
                title=title,
    ...
    def commit_reservation(self, token: str) -> dict[str, object]:
        with self._lock:
            ...
            self._config = self._consume_reservation(reservation)
            self._save_config(self._config)
            return self._serialize_status_locked()
    ...
    def _build_next_config(
        ...
        *,
        ...
        next_used = 0
        if normalized_mode == MODE_DAILY and self._config.mode == MODE_DAILY:
            next_used = max(0, min(self._config.daily_used, max(0, int(daily_limit))))
    ...
    def _consume_reservation(self, reservation: QuotaReservation) -> PublicPanelConfig:
    ...
    def _serialize_status_locked(self) -> dict[str, object]:
        available_quota = max(0, self._available_quota_locked())
        disabled_reason = None
        if not self._config.enabled:
            disabled_reason = "disabled"
        elif available_quota <= 0:
            disabled_reason = "quota_exhausted"
        payload = asdict(self._config)
        payload["available_quota"] = available_quota
        payload["quota"] = available_quota
    ...
    def _pop_reservation_locked(self, token: str) -> QuotaReservation:
        reservation = self._reservations.pop(str(token or "").strip(), None)
